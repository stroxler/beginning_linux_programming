...
/* We then define the named pipes. We use one pipe for the server and one pipe
 * for each client.  Since there may be multiple clients, the client
 * incorporates a process ID into the name to ensure that its pipe is unique.
*/
...
/*  Finally, we get to the pipe interface functions that perform data transfer
 *  implementedg in pipe_imp.c. These divide into server- and client-side
